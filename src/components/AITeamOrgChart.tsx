    ...
    <section className="py-[80px] md:py-[120px]">
      <div className="max-w-[1280px] mx-auto px-5 md:px-[clamp(20px,5vw,80px)]">
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={0}
          ...
          <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-white/50 mb-4">Équipe AI</p>
          <h2 className="text-[24px] md:text-[clamp(32px,4vw,44px)] font-medium tracking-[-0.03em] mb-4 leading-[1.15] text-white">
            <span className="font-['Instrument_Serif'] italic">Lystes {productCount} produits</span>{" "}
            <span className="inline-block">= <span className="font-['Instrument_Serif'] italic text-white/50">{totalAgents} agents dédiés</span></span>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={1}
          ...
          <div className="flex items-center justify-between text-[12px] text-white/50 mb-2">
            ...
            <span className="font-medium text-white">{productCount} produits</span>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={2}
          ...
          <div className="max-w-[1100px] mx-auto flex flex-col items-center">
            ...
            {/* MyGPT */}
            <div className="bg-white text-foreground px-5 md:px-8 py-4 md:py-5 rounded-xl flex items-center justify-between gap-4 md:gap-6 w-full max-w-[400px] transition-all">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  <h4 className="text-base font-semibold text-foreground">MyGPT</h4>
            ...
            <div className="w-px bg-white/20 mx-auto" style={{ height: 40 }} />

            {/* Directors label */}
            <div className="text-center mb-5 mt-2">
              <span className="text-[10px] font-semibold uppercase tracking-[0.12em] text-white/50">Directeurs</span>
            </div>

            {/* Directors carousel - Desktop */}
            <div className="hidden md:flex gap-3 justify-center flex-wrap items-center">
              <button onClick={() => navDir(-1)} className="w-9 h-9 rounded-full border border-white/20 bg-white/5 flex items-center justify-center text-base text-white/60 transition-all hover:bg-white hover:text-foreground shrink-0">←</button>
              {directors.map((d, i) => (
                <button key={d.name} onClick={() => setActiveDir(i)}
                  className={`px-5 py-4 rounded-xl text-center w-[160px] relative transition-all duration-[350ms] ${
                    i === activeDir
                      ? "bg-white text-foreground border border-white -translate-y-1 scale-[1.03] shadow-[0_8px_24px_rgba(0,0,0,0.3)]"
                      : "bg-white/5 text-white border border-white/20 hover:border-white/40 hover:-translate-y-0.5"
                  }`}>
                  <img src={d.img} alt={d.name} className={`w-10 h-10 rounded-full mx-auto mb-2 border border-white/20 ${i === activeDir ? "bg-white" : "bg-white/10 invert"}`} />
                  <h4 className="text-[13px] font-semibold mb-0.5">{d.name}</h4>
                  <span className={`text-[9px] block mb-2 ${i === activeDir ? "text-muted-foreground" : "text-white/50"}`}>{d.role}</span>
                  <span className={`text-[7px] font-bold uppercase tracking-[0.08em] px-2 py-0.5 border rounded inline-block ${
                    i === activeDir ? "border-border text-muted-foreground" : "border-white/20 text-white/40"
                  }`}>{d.teamLabel}</span>
                  {i === activeDir && <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-white" />}
                </button>
              ))}
              <button onClick={() => navDir(1)} className="w-9 h-9 rounded-full border border-white/20 bg-white/5 flex items-center justify-center text-base text-white/60 transition-all hover:bg-white hover:text-foreground shrink-0">→</button>
            </div>
            ...
            <div className="flex md:hidden gap-4 justify-center items-center">
              <button onClick={() => navDir(-1)} className="w-9 h-9 rounded-full border border-white/20 bg-white/5 flex items-center justify-center text-white/60 shrink-0">←</button>
              <div className="px-5 py-4 rounded-xl text-center w-[160px] bg-white text-foreground border border-white -translate-y-1 scale-[1.03] shadow-[0_8px_24px_rgba(0,0,0,0.3)]">
            ...
            <div className="flex gap-1.5 justify-center my-3">
              {directors.map((_, i) => (
                <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i === activeDir ? "w-5 bg-white" : "w-1.5 bg-white/20"}`} />
              ))}
            </div>

            <div className="w-px bg-white/20 mx-auto" style={{ height: 24 }} />
            ...
            <div className="w-full max-w-[900px] transition-opacity duration-300" key={activeDir}>
              ...
              <div className="flex gap-3 justify-center flex-wrap mb-2">
                {dir.managers.map(m => (
                  <div key={m.name} className="px-4 py-3.5 rounded-xl text-center w-[110px] bg-white/5 text-white border border-white/20 transition-all duration-200">
                    <img src={m.img} alt={m.name} className="w-9 h-9 rounded-full mx-auto mb-2 bg-white/10 border border-white/20 invert" />
                    <h4 className="text-xs font-semibold mb-0.5 truncate">{m.name}</h4>
                    <span className="text-[9px] block mb-2 truncate text-white/50">{m.role}</span>
                    <div className="text-[7px] font-bold uppercase tracking-[0.08em] px-1.5 py-0.5 border rounded inline-block border-white/20 text-white/40">{m.badge}</div>
                  </div>
                ))}
              </div>
              ...
              <div className={`grid gap-5 mx-auto ${dir.teams.length === 1 ? "grid-cols-1 max-w-[300px]" : dir.teams.length === 2 ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1 md:grid-cols-3"}`}>
                {dir.teams.map(t => {
                  ...
                  return (
                    <div key={t.name} className="p-6 border border-dashed border-white/20 rounded-2xl bg-white/5 text-center transition-all w-full">
                      <div className="flex items-center justify-center gap-1 mb-3">
                        <div className="flex ml-2">
                          {[...Array(shown)].map((_, j) => (
                            <div key={j} className="w-[30px] h-[30px] rounded-full bg-white/10 border-2 border-foreground flex items-center justify-center text-[10px] font-medium text-white/60"
                              style={{ marginLeft: j > 0 ? -8 : 0, zIndex: shown - j }}>
                              {j + 1}
                            </div>
                          ))}
                        </div>
                        {extra > 0 && <span className="text-[11px] text-white/50 ml-1">+{extra}</span>}
                      </div>
                      <h4 className="text-[15px] font-semibold text-white mb-1">{t.name}</h4>
                      <p className="text-[13px] text-white/50 mb-2.5">{count} {count > 1 ? "créateurs" : "créateur"}</p>
            ...
            <div className="mt-12 w-full max-w-[900px]">
              <div className="w-px bg-white/20 mx-auto" style={{ height: 40 }} />
              <div className="bg-white/10 border border-white/20 rounded-2xl px-6 md:px-10 py-6 md:py-7 flex flex-col md:flex-row items-center gap-5 md:gap-10 max-w-[800px] mx-auto">
