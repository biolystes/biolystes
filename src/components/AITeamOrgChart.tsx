      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={2}
        ...
        <div className="max-w-[1100px] mx-auto flex flex-col items-center">
          ...
          <div className="hidden md:flex gap-3 justify-center flex-wrap items-center">
            <button onClick={() => navDir(-1)} className="w-9 h-9 rounded-full border border-white/20 flex items-center justify-center text-base text-white/50 transition-all hover:bg-white hover:text-foreground shrink-0">←</button>
            {directors.map((d, i) => (
              <button key={d.name} onClick={() => setActiveDir(i)}
                className={`px-5 py-4 rounded-xl text-center w-[160px] relative transition-all duration-[350ms] ${
                  i === activeDir
                    ? "bg-white text-foreground border border-white -translate-y-1 scale-[1.03] shadow-[0_8px_24px_rgba(0,0,0,0.12)]"
                    : "bg-white/5 text-white border border-white/20 hover:border-white/40 hover:-translate-y-0.5"
                }`}>
                <img src={d.img} alt={d.name} className="w-10 h-10 rounded-full mx-auto mb-2 bg-white border border-white/20" />
          ...
          <div className="flex md:hidden gap-4 justify-center items-center">
            <button onClick={() => navDir(-1)} className="w-9 h-9 rounded-full border border-white/20 flex items-center justify-center text-white/50 shrink-0">←</button>
            <div className="px-5 py-4 rounded-xl text-center w-[160px] bg-white text-foreground border border-white -translate-y-1 scale-[1.03] shadow-[0_8px_24px_rgba(0,0,0,0.12)]">
              <img src={dir.img} alt={dir.name} className="w-10 h-10 rounded-full mx-auto mb-2 bg-white border border-white/20" />
          ...
          <div className="w-full max-w-[900px] transition-opacity duration-300" key={activeDir}>
            ...
            <div className="flex gap-3 justify-center flex-wrap mb-2">
              {dir.managers.map(m => (
                <div key={m.name} className="px-4 py-3.5 rounded-xl text-center w-[110px] bg-white/5 text-white border border-white/15 transition-all duration-200">
                  <img src={m.img} alt={m.name} className="w-9 h-9 rounded-full mx-auto mb-2 bg-white border border-white/20" />
            ...
            <div className={`grid gap-5 mx-auto ${dir.teams.length === 1 ? "grid-cols-1 max-w-[300px]" : dir.teams.length === 2 ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1 md:grid-cols-3"}`}>
              {dir.teams.map(t => {
                ...
                return (
                  <div key={t.name} className="p-6 border border-dashed border-white/15 rounded-2xl text-center transition-all w-full">
          ...
          <div className="mt-12 w-full max-w-[900px]">
            <div className="w-px bg-white/20 mx-auto" style={{ height: 40 }} />
            <div className="bg-white text-foreground rounded-2xl px-6 md:px-10 py-6 md:py-7 flex flex-col md:flex-row items-center gap-5 md:gap-10 max-w-[800px] mx-auto">
